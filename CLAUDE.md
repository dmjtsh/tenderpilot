...
- **AI резюме** (`apps/tenders/services.py`)
  - generate_tender_summary(tender) — DeepSeek V4 Flash (deepseek-chat) с fallback на GPT-4o-mini
  - get_or_create_summary(tender) — кэш в tender.ai_summary, lazy regen при version != 2
  - Промпт v2: структурированный JSON (customer_analysis, work_description, key_risks, required_documents)
...
### Данные (прод, май 2026)
- ~130k тендеров, ~122k обогащено
- Активных ~44k (остальные finished/expired)
...
- `apps/tenders/services.py:generate_tender_summary()` — рабочая функция AI-резюме на DeepSeek
...
| gpt-4o-mini | $0.15 | $0.60 | HyDE, классификация документов, fallback |
