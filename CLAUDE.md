...
/backend
  /apps
    /tenders      — парсинг ЕИС, TenderGuru API, модели тендеров, справочник ОКВЭД, pipeline
...
- **TenderGuru API** (`apps/tenders/tenderguru_client.py`)
  - API v2.3, `api_code` auth, base URL `https://www.tenderguru.ru/api2.3/export`
  - `search_tenders()` — список, фильтры: `f=kom|44|223`, `price1/price2`, `actual`, пагинация
  - `fetch_tender_detail()` — детализация: TenderType, CustomerINN, docsXML, productsXML, Info HTML
  - `parse_list_item()` + `enrich_from_detail()` — парсинг в upsert_tender-compatible dict
  - `_parse_info_html()` — извлекает delivery_place, security_amount, procedure_type из HTML
  - Procedure type detection: title keywords → searchFragmentXML → Info HTML → TenderType
  - CustomerINN валидация: API иногда возвращает название компании вместо ИНН
  - Коммерческие тендеры: sparse data (без цены, региона, доков на мелких площадках)
  - Документы: docsXML (Фабрикант) + linksTenderXML (Бидзаар, B2B-Center, Портал поставщиков). РТС-Маркет/ОТС/СберБ2Б — без доков
  - Bulk load: `python manage.py bulk_load_tenderguru --limit=5000 --delay=0.3`
  - Sync: `sync_tenderguru` Celery task, 5000/run, каждые 2 часа
  - Env: `TENDERGURU_API_KEY` (без ключа API отдаёт 10 записей на 1 странице)
...
#### Компоненты
- `TenderCard` — карточка тендера, cosine score + score_label, b2b badge с названием площадки (`_shortPlatform` + PLATFORM_RULES), НМЦК "Не указана" для null
...
### Данные (прод, 25 мая 2026)
- ~133k тендеров (active: 58.9k 44-ФЗ + 15k 223-ФЗ ≈ 74k active)
- TenderGuru: bulk load в процессе (коммерческие b2b тендеры)
...
- TenderGuru: ~15k тендеров (b2b + 44/223-ФЗ с коммерческих площадок)
...
| `sync_tenderguru` | каждые 2ч в :15 | Коммерческие тендеры из TenderGuru API (5000/run) |
| `check_pipeline_health` | каждые 15 мин | Алерт если sync не запускался >2ч |
...
- Сервисы: `tenderpilot-web`, `tenderpilot-worker`, `tenderpilot-beat`, `tenderpilot-frontend` (systemd)
...
# Bulk load ЕИС (одноразовый, подтягивает все тендеры за N дней)
python manage.py bulk_load_tenders --days=90 --delay=0.5
python manage.py bulk_load_tenders --days=7 --no-enrich  # без обогащения

# Bulk load TenderGuru (коммерческие тендеры, запускать в tmux)
python manage.py bulk_load_tenderguru --limit=5000 --delay=0.3
python manage.py bulk_load_tenderguru --limit=10 --dry-run  # только подсчёт
python manage.py bulk_load_tenderguru --law=44 --limit=100  # 44-ФЗ тендеры
...
- [x] Фильтр по площадкам (icontains, 19 площадок, работает на обоих табах)
...
- [x] TenderGuru API интеграция (коммерческие тендеры, bulk load, sync каждые 2ч)
- [x] UI фильтры переопределяют настройки направлений (law_type, region, procedure_type)
